...
public class JyouchiReiEffect extends AbstractGameEffect
{
    private float x;
    private float y;
    private float vX;
    ...
    public JyouchiReiEffect(Color setColor, Hitbox hb)
    {
...
//        this.y = (MathUtils.random(this.starter.y - this.hbR, this.starter.y + this.hbR) - this.img.packedHeight / 2.0F);
    ...
    public void update()
    {
        ...
        if (this.trailTimer < 0.0F)
        {
            ...
            this.prevPositions.add(new Vector2(this.fX, this.fY));
...
//        this.fY = (float) (Math.sin(this.ro) * (this.x - this.starter.x) + Math.sin(ro2) * (this.y - this.starter.y)) + this.starter.y;
        ...
        if (this.duration > this.startingDuration / 2.0F)
        {
            ...
            this.color.a = (Interpolation.fade.apply(0.5F, 1.0F, this.startingDuration / 2.0F - tmp) * this.baseAlpha);
        }
        else
        {
            this.color.a = (Interpolation.fade.apply(0.5F, 1.0F, this.duration / (this.startingDuration / 2.0F)) * this.baseAlpha);
        }
    }
    ...
    private void changeRotation(){
        ...
        if(this.renderBehind){
            this.sca = Interpolation.fade.apply(2.5F, 3.0F, (float) range / this.hbR);
        }
        else {
            this.sca = Interpolation.fade.apply(3.5F, 3.0F, (float) range / this.hbR);
        }

        this.vX = Interpolation.fade.apply(this.vXnum, this.vXnum / 3, (float) range / this.hbR);
    }
